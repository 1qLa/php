...
try {
  ...
  $where = "";
  if($searchType == 1){
    $where = " where price >= :pr";
  }else if($searchType == 2){
    $where = " where category = :ca";
  }
  ...
  if($searchType == 1){
    ...
    $stml = $db->prepare($sql);
    $stml -> bindValue(':pr', $condition, PDO::PARAM_INT);
  }else if($searchType == 2 && $category != 1){
    $sql = "SELECT * FROM oldProduct".$where;
    $stml = $db->prepare($sql);
    switch($category){
      case 2:
    ...
    $stml  -> bindValue(':ca', $condition, PDO::PARAM_INT);
  }
  $stml->execute();
      ...
      <div class="col-3 border">
        <form action="kadai08_1.php" method="GET" class="mt-5 m-3">
          ...
          <div class="form-check form-check-inline mb-3">
            <input class="form-check-input" type="radio" name="searchType" id="searchRadio1" value="1" <?php if($searchType == 1) echo "checked = 'checked'" ?> onclick="typeCheck();">
            <label class=" form-check-label" for="searchRadio1">価格検索</label>
          </div>
          <div class="form-check form-check-inline mb-3">
            <input class="form-check-input" type="radio" name="searchType" id="searchRadio2" value="2" <?php if($searchType == 2) echo "checked = 'checked'" ?> onclick="typeCheck();">
          ...
          <div class="input-group mb-3">
            ...
            <option value="1" <?php if($category == 1) echo "selected"; ?>>全ての商品</option>
        ...
        <table class="table table-hover mt-5 form-control-lg">
          ...
          <?php foreach($result as $v): ?>
            <tr>
              ...
              <td><a class="btn btn-primary" href="kadai11_1.php?product_no=<?= $v["product_no"] ?>">編集</a></td>
              <td><a class="btn btn-secondary" href="kadai11_3.php?product_no=<?= $v["product_no"] ?>">削除</a></td>
            </tr>
          <?php endforeach; ?>
